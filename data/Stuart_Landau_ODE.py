...
def Stuart_Landau_ODE(x1range, x2range, numICs, tSpan, seed, type="z"):  # function X = PendulumFn(x1range, x2range, numICs, tSpan, seed, max_potential)
    ...
    def dynsys(x, t):
        ...
        dydt[0] = x[0] - x[1] - x[0] * (x[0] ** 2 + x[1] ** 2)  # x[1, :]
        dydt[1] = x[0] + x[1] - x[1] * (x[0] ** 2 + x[1] ** 2)
        # print(dydt)
        return dydt
