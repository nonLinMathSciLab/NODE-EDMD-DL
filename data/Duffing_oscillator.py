...
X_train = Duffing_oscillator_ODE(x1range, x2range, numICs, tSpan, seed, "x") # 0, 2.5, 0.25
filename_train = filenamePrefix + '_train_x.csv'
make_csv(filename_train, X_train)

#seed = 10
tSpan = np.arange(0, 2.5 + 0.1, 0.25) # 0, 2.5 + 0.1, 0.25
X_train = Duffing_oscillator_ODE(x1range, x2range, numICs, tSpan, seed, "y")
filename_train = filenamePrefix + '_train_y.csv'
make_csv(filename_train, X_train)

#seed = 1#1
tSpan = np.arange(0, 12.5, 0.25)
X_train = Duffing_oscillator_ODE(x1range, x2range, numICs, tSpan, seed)
...
seed = 3
tSpan = np.arange(0, 0.25 + 0.1, 0.25)  # 0, 12.5, 0.25
X_train = Duffing_oscillator_ODE(x1range, x2range, 10000, tSpan, seed)
filename_train = filenamePrefix + "_E_eigfunc" + '.csv'
make_csv(filename_train, X_train)
seed = 1
