...
def graph(x, y, name, type, correct=[], predict=[], phi_predict=[]):  # plt.xlim(1300,)
    ...
    if type == "plot":
        ...
        #plt.scatter([i for i in range(50)], phi_predict, label="predicted", color="orange")
        #plt.plot(phi_predict, label="predict")  # 予測Φ，緑
        #plt.title(name[:1] + "_trajectory")
        plt.xlabel('n')
        plt.ylabel(name[:1])
        plt.legend()
    ...
    plt.savefig("MLP_img/" + data_name + "_png/" + name + ".png")
    plt.savefig("MLP_img/" + data_name + "_eps/" + name + ".eps")
    #plt.show()
...
loss = 10000000#18.5135
min_loss = loss
cnt = 0
while loss > epsilon and cnt <= rotation:  # count < rotation and
    cnt += 1
    if count % 100 == 0:
        print(count)
    ...
    if loss < min_loss:  # lowest_lossのときのNNを記録
        ...
        min_loss = loss
    with open('p_MLP_net_latest.pkl', 'wb') as f:
        cloudpickle.dump(net, f)
    ...
    G = inv_N * torch.mm(pred_sai_T, pred_sai)
    A = inv_N * torch.mm(pred_sai_T, y_pred_sai)

    #K_tilde = torch.mm(p_inv(G + lambda_ * I), A)
...
for tr_val_te in ["E_recon_50"]:
    ...
    B = torch.tensor([[1 if ((i == 23 and j == 0) or (i == 24 and j == 1)) else 0 for i in range(M + 3)] for j in range(2)])
    B = B.detach().numpy()
    K = K.detach().numpy()

    data = data_Preprocessing("E_recon_50", 100000)
    width = 50
    ...
    while count < 2000:#10:
        ...
        E_recon = (inv_N * sum([abs(x_data[n][0] - x_tilde_phi[n][0]) ** 2 + abs(x_data[n][1] - x_tilde_phi[n][1]) ** 2
        ...
        graph([], [], "x_traj_" + "{stp:02}".format(stp=count), "multi_plot"
              , x_data[:, 0], x_tilde_0, x_tilde_phi_0)

        x_tilde_1 = [j for i, j in x_tilde]
        x_tilde_phi_1 = [j for i, j in x_tilde_phi]

        graph([], [], "y_traj_" + "{stp:02}".format(stp=count), "multi_plot"
              , x_data[:, 1], x_tilde_1, x_tilde_phi_1)

        # converge zone
        x_start_list.append(x_tilde_0[0])
        x2_start_list.append(x_tilde_1[0])
        x_lim_list.append(x_tilde_phi_0[width - 1])
        x_exact_lim_list.append(x_data[width - 1][0])

        # Stuart_Landau xy picture
        if count == 1:
            plt.figure()
            plt.scatter(x_tilde_phi_0, x_tilde_phi_1, label="exact")
            plt.title("trajectory")
            plt.xlabel('x')
            plt.ylabel('y')
            plt.legend()
            plt.tight_layout()
            plt.savefig("MLP2_img/" + data_name + "_png/" + "trajectory.png")
            plt.savefig("MLP2_img/" + data_name + "_eps/" + "trajectory.eps")

# converge zone
x_lim_list = discrimination(x_lim_list)
x_exact_lim_list = discrimination(x_exact_lim_list)

score = True_or_False(x_exact_lim_list, x_lim_list)
print(score)

converge_plot("MLP", "exact", x_start_list, x2_start_list, x_lim_list)
converge_plot("MLP", "predicted", x_start_list, x2_start_list, x_exact_lim_list)

"""学習済みのnetを使って，E_eigfuncを計算"""
I_number = 10000
data = data_Preprocessing("E_eigfunc", I_number * width)
width = 2
phi_list = [[0 for count in range(I_number)] for j in range(M + 3)]
y_phi_list = [[0 for count in range(I_number)] for j in range(M + 3)]
...
"""固有関数，規格化"""
for j in range(M + 3):
    tmp = sum([(phi_list[j][count]).real ** 2 + (phi_list[j][count]).imag ** 2 for count in range(I_number)]) ** 0.5
    y_tmp = sum(
        [(y_phi_list[j][count]).real ** 2 + (y_phi_list[j][count]).imag ** 2 for count in range(I_number)]) ** 0.5
    for count in range(I_number):
        phi_list[j][count] = phi_list[j][count] / tmp #** 0.5
        y_phi_list[j][count] = y_phi_list[j][count] / y_tmp #** 0.5
