...
with open('p200_3_75_MLP_net.pkl', 'rb') as f:  # nolta_
...
min_loss = loss
cnt = 0
while loss > epsilon and cnt == 0:  # count < rotation and
    cnt += 1
    if count % 100 == 0:
        print(count)

    """if loss < min_loss:  # lowest_lossのときのNNを記録
        with open('p200_3_MLP_net.pkl', 'wb') as f:
            cloudpickle.dump(net, f)
        min_loss = loss
    with open('p200_3_MLP_net_latest.pkl', 'wb') as f:
        cloudpickle.dump(net, f)"""
